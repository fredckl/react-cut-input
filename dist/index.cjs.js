...
var formatSizes = function formatSizes(sizes) {
  return rambda.reduce(function (acc, value) {
    if (rambda.is(Number, value)) return [].concat(_toConsumableArray(acc), [_objectSpread2(_objectSpread2({}, defaultSizes), {}, {
      maxLength: value
    })]);
...
var formatDefaultValues = function formatDefaultValues(sizes) {
  var count = 0;
  return rambda.reduce(function (acc) {
...
var formatRegexByItem = function formatRegexByItem(item, defaultPattern, exactPattern) {
  return rambda.prop(PATTERN, item) ? "(".concat(rambda.prop(PATTERN, item), ")") : "(".concat(defaultPattern || '.', "{").concat(rambda.prop(MAX_LENGTH, item), "})").concat(!exactPattern ? '?' : '');
};
/**
...
var formatValues = function formatValues(value, sizes, defaultPattern, exactPattern) {
  var regex = rambda.compose(rambda.join(''), rambda.map(function (item) {
...
var useCutInput = function useCutInput(_ref) {
  ...
  var _useState = React.useState(_objectSpread2(_objectSpread2({}, formatDefaultValues(newSizes)), formatValues(value, newSizes, pattern, exactPattern))),
      _useState2 = _slicedToArray(_useState, 2),
      state = _useState2[0],
      setState = _useState2[1];

  var getSizeProp = function getSizeProp(arrayPath) {
    return rambda.path(arrayPath)(newSizes);
  };

  var getMaxLength = function getMaxLength(index) {
    return getSizeProp([index, MAX_LENGTH]);
  };

  var getValidator = function getValidator(index) {
    return getSizeProp([index, VALIDATOR]);
  };

  var getTransform = function getTransform(index) {
    return getSizeProp([index, TRANSFORM]);
  };

  var getValues = function getValues(value) {
    return rambda.values(value);
  };

  var concatValues = function concatValues(value) {
    return rambda.join('', getValues(value));
  };

  var handleChange = function handleChange(value, index) {
    if (!checkMaxLength(value, getMaxLength(index))) return null;
    var newValue = checkValidator(value, index);
    newValue = transformValue(newValue, index);
    var mergedValues = rambda.mergeLeft(_defineProperty({}, getInputName(index), newValue), state);
    setState(mergedValues);
    var children = rambda.pathOr([], ['current', 'children'], ref); // Go to next input

    rambda.when(rambda.both(rambda.prop(index + 1), function () {
      return rambda.length(newValue) === getMaxLength(index);
    }), function () {
      return children[index + 1].focus();
    })(rambda.pathOr([], ['current', 'children'], ref));

    if (rambda.is(Function, onChange)) {
      onChange(concatValues(mergedValues), getValues(mergedValues));
    }
  };

  var checkMaxLength = function checkMaxLength(value, len) {
    return value.length <= len;
  };

  var checkValidator = function checkValidator(value, index) {
    var regex = getValidator(index) || validator;

    if (value && regex && !new RegExp(regex, 'gi').test(value)) {
      return rambda.prop(getInputName(index), state);
    }

    return value;
  };

  var transformValue = function transformValue(value, index) {
    transform = getTransform(index) || transform;

    if (rambda.is(Function, transform)) {
      return transform(value);
    }

    if (rambda.is(String, transform)) {
      var newValue;

      switch (transform) {
        case 'uppercase':
          newValue = rambda.toUpper(value);
          break;

        case 'lowercase':
          newValue = rambda.toLower(value);
          break;
...
var CutInput = function CutInput(_ref) {
  ...
  return /*#__PURE__*/React__default['default'].createElement("div", {
    ref: ref,
    className: "split-input-container ".concat(className)
  }, Object.keys(state).map(function (v, i) {
    return /*#__PURE__*/React__default['default'].createElement(Input, {
      key: i,
      value: state[getInputName(i)],
      onChange: function onChange(e) {
        return handleChange(e.target.value, i);
      }
    });
  }));
};
