...
var useCutInput = function useCutInput(_ref) {
  ...
  var getSizeProp = function getSizeProp(arrayPath) {
    return path(arrayPath)(newSizes);
  };
  ...
  var handleChange = function handleChange(value, index) {
    ...
    var mergedValues = mergeLeft(_defineProperty({}, getInputName(index), newValue), state);
    setState(mergedValues);
    var children = pathOr([], ['current', 'children'], ref); // Go to next input

    when(both(prop(index + 1), function () {
      return length(newValue) === getMaxLength(index);
    }), function () {
